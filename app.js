...
function calc(){return `<div class="card"><h2>Calculadores de 2n BATX</h2><p>Selecciona una eina ràpida. Les calculadores són de suport i no substitueixen la resolució escrita.</p><label>Tipus<select id="calcType"><option value="torque">Parell i potència</option><option value="heat">Calor Q=m·ce·ΔT</option><option value="gas">Gas ideal pV=nRT</option><option value="hyd">Cilindre hidràulic F=p·A</option><option value="rl">Reactància inductiva XL</option><option value="tri">Trifàsica P=√3·U·I·cosφ</option></select></label><div id="calcFields" class="grid"></div><button class="btn primary" onclick="runCalc()">Calcular amb explicació</button><div id="calcOut" class="result"></div></div>`}
function setCalcFields(){let t=$('#calcType')?.value;if(!t)return;let html='';let fields={torque:[['P','Potència (W)',3000],['n','Velocitat (min⁻¹)',1450]],heat:[['m','Massa (kg)',10],['ce','Calor específica (J/kg·K)',4180],['dt','Increment T (K)',40]],gas:[['n','Mols',1],['T','Temperatura (K)',293],['V','Volum (m³)',0.02]],hyd:[['p','Pressió (bar)',80],['d','Diàmetre (mm)',50]],rl:[['f','Freqüència (Hz)',50],['L','Inductància (H)',0.2]],tri:[['U','Tensió composta (V)',400],['I','Intensitat (A)',10],['c','cos φ',0.8]]}[t]; fields.forEach(([id,lab,val])=>html+=`<label>${lab}<input id="c_${id}" type="number" step="any" value="${val}"></label>`); $('#calcFields').innerHTML=html}
...
function render(){nav(); let v=state.view; $('#app').innerHTML = v=='inici'?inicio():v=='temes'?temes():v=='exercicis'?exerciseView():v=='calculadores'?calc():v=='pràctica'?practica():v=='docent'?docent():v=='progrés'?progres():v=='auditoria'?auditoria():ajuda(); if(v=='calculadores'){setCalcFields(); $('#calcType').addEventListener('change',setCalcFields)}}
render();
